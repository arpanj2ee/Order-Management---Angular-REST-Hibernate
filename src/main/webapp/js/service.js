...
orderApp.factory('OrderService',['$http','$q',function($http,$q){
    var REST_SERVICE_URI = 'http://localhost:8080/order-management/orders';
    ...
    function fetchAllOrders() {
     ...
     function fetchOrder() {
        var deferred = $q.defer();
        $http.get(REST_SERVICE_URI+"/"+id,{params:{"id":id}})
            .then(
            function (response) {
                deferred.resolve(response.data);
            },
    ...
    function updateOrder(order, id) {
        var deferred = $q.defer();
        $http.put(REST_SERVICE_URI+"/"+id, order,{params:{"id":id}})
            .then(
            function (response) {
                deferred.resolve(response.data);
            },
    ...
    function deleteOrder(id) {
        var deferred = $q.defer();
        $http.delete(REST_SERVICE_URI+"/"+id,{params:{"id":id}})
            .then(
            function (response) {
                deferred.resolve(response.data);
            },
